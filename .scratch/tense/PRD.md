...
Agents interact through three MCP tools — `remember`, `recall(as_of?)`, and `history` — over stdio, so Tense plugs into Claude Code or Cursor. A live read-only Next.js viewer renders the graph and animates Supersession in real time (Current Facts solid, superseded Facts greyed/dashed). The flagship demonstration feeds two conflicting Sources and shows the graph re-wire live, then contrasts Tense's correct point-in-time answers against a vector-only baseline that cannot tell Current from stale.
