...
<img src="docs/media/accuracy.svg" alt="Grouped bar chart of offline temporal-QA accuracy: on the 5 point-in-time questions Tense scores 100% and the fair vector baseline 0%; across all 11 questions Tense scores 100% and the baseline 45.5% — the baseline competes on the 'now' questions and loses precisely the 5 whose answer changed over time." width="100%">

That chart is regenerated by the same `pnpm eval:report` run that writes
[`eval/RESULTS.md`](./eval/RESULTS.md), and a drift-guard test
([`test/eval-results-snapshot.integration.test.ts`](./test/eval-results-snapshot.integration.test.ts))
fails if either falls out of sync with a live offline run — so the bars cannot
quietly drift from the numbers. It runs 9
...
`pnpm test` runs **237 tests across 47 spec files** (~15s locally, all green;
3 environment-gated tests skip without their keys). The number
