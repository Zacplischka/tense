const FACT_COLUMNS = "id, subject_id, predicate, object_id, source_id, valid_at, invalid_at, created_at, expired_at";
...
const FACT_TSVECTOR = "to_tsvector('english', subj.name || ' ' || replace(f.predicate, '-', ' ') || ' ' || obj.name)";
function mapRecalledRow(row) {
    return {
        id: row.id,
        ...
        learnedAt: row.tx_created,
        source: {
            id: row.source_id,
            label: row.source_label ?? null,
            text: row.source_text,
        },
        reinforcedBy: row.reinforced_by ?? 0,
    };
}
/**
 ...
 */
function mapFactChange(row) {
    return { ...mapRecalledRow(row), retiredAt: row.expired_at ?? null };
}
/**
...
export class TemporalGraphStore {
    ...
    async insertSource(text, label = null) {
        const { rows } = await this.pool.query("INSERT INTO sources (text, label) VALUES ($1, $2) RETURNING id, label, text, created_at", [text, label]);
     ...
     */
    async upsertEntity(name) {
        const normalized = normalizeName(name);
        const existing = await this.pool.query("SELECT id, name, normalized_name, created_at FROM entities WHERE normalized_name = $1", [normalized]);
        if (existing.rowCount)
            return mapEntity(existing.rows[0]);
        const { rows } = await this.pool.query(`INSERT INTO entities (name, normalized_name) VALUES ($1, $2)
       ON CONFLICT (normalized_name) DO UPDATE SET name = entities.name
       RETURNING id, name, normalized_name, created_at`, [name, normalized]);
        return mapEntity(rows[0]);
    }
    async insertFact(fact) {
        const { rows } = await this.pool.query(`INSERT INTO facts (subject_id, predicate, object_id, source_id, valid_at, invalid_at, expired_at)
    ...
    async currentFactsForSubject(subjectId) {
        const { rows } = await this.pool.query(`SELECT f.id, f.predicate, obj.name AS object, f.valid_at, f.created_at
       FROM facts f JOIN entities obj ON obj.id = f.object_id
       WHERE f.subject_id = $1 AND f.expired_at IS NULL
       ORDER BY f.created_at ASC`, [subjectId]);
        return rows.map((r) => ({
            id: r.id,
     ...
     * transaction itself.
     */
    async closeFactsTx(client, closes) {
        const closed = [];
        for (const close of closes) {
            const { rows } = await client.query(`UPDATE facts SET invalid_at = $2, expired_at = $3
         WHERE id = $1 AND expired_at IS NULL
         RETURNING ${FACT_COLUMNS}`, [close.factId, close.invalidAt, close.expiredAt]);
            if (rows[0])
                closed.push(mapFact(rows[0]));
        }
        return closed;
    }
    /** Atomically close (expire) a set of Facts — used by the contradiction path. */
    async expireFacts(closes) {
        if (closes.length === 0)
            return [];
    ...
    async currentFactsFor(subjectId, predicate) {
        const { rows } = await this.pool.query(`SELECT ${FACT_COLUMNS} FROM facts
       WHERE subject_id = $1 AND predicate = $2 AND expired_at IS NULL
       ORDER BY created_at ASC`, [subjectId, predicate]);
     ...
     */
    async supersedeAndInsert(closes, newFact) {
        const client = await this.pool.connect();
        try {
            await client.query("BEGIN");
            const closed = await this.closeFactsTx(client, closes);
            const { rows } = await client.query(`INSERT INTO facts (subject_id, predicate, object_id, source_id, valid_at, invalid_at, expired_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         RETURNING ${FACT_COLUMNS}`, [
                newFact.subjectId,
                newFact.predicate,
    ...
    async setFactEmbedding(factId, embedding) {
    ...
    async hasEmbedding(factId) {
        const { rows } = await this.pool.query("SELECT embedding IS NOT NULL AS has FROM facts WHERE id = $1", [factId]);
     ...
     */
    async addFactSource(factId, sourceId) {
        await this.pool.query("INSERT INTO fact_sources (fact_id, source_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", [factId, sourceId]);
    }
    /** How many distinct Sources have asserted a Fact ("reinforced N times"). */
    async countFactSources(factId) {
        const { rows } = await this.pool.query("SELECT count(*)::int AS n FROM fact_sources WHERE fact_id = $1", [factId]);
        return rows[0]?.n ?? 0;
    }
    async getEntity(id) {
        const { rows } = await this.pool.query("SELECT id, name, normalized_name, created_at FROM entities WHERE id = $1", [id]);
        return rows[0] ? mapEntity(rows[0]) : null;
    }
    /** Entity names (capped) to hint extraction toward reusing existing Entities. */
    async listEntityNames(limit = 200) {
     ...
     */
    async changesSince(since, limit = 50) {
        const lim = clampLimit(limit);
        const { rows } = await this.pool.query(`${RECALL_SELECT}
     ...
     */
    async recallByTemporal(asOf, limit = 50, predicate = null, minReinforced = null) {
        const lim = clampLimit(limit);
        const params = [];
        const clauses = [];
        if (asOf === null) {
        ...
        if (predicate)
            clauses.push(`f.predicate = $${params.push(predicate)}`);
        ...
        const { rows } = await this.pool.query(`${RECALL_SELECT} WHERE ${clauses.join(" AND ")} ORDER BY ${order} LIMIT ${lim}`, params);
        return rows.map(mapRecalledRow);
    }
    /** Rank Fact ids by semantic similarity to a query embedding, temporally filtered. */
    async rankBySemantic(queryEmbedding, asOf, limit = 20, predicate = null, minReinforced = null) {
        const lim = clampLimit(limit);
        const params = [formatVector(queryEmbedding)]; // $1 = query vector
        ...
        if (predicate)
            clauses.push(`predicate = $${params.push(predicate)}`);
        if (minReinforced && minReinforced > 0)
            clauses.push(`(SELECT count(*) FROM fact_sources fs WHERE fs.fact_id = facts.id) >= $${params.push(minReinforced)}`);
        const { rows } = await this.pool.query(`SELECT id FROM facts WHERE ${clauses.join(" AND ")} ORDER BY embedding <=> $1::vector LIMIT ${lim}`, params);
        return rows.map((r) => r.id);
    }
    /** Rank Fact ids by Postgres full-text relevance, temporally filtered. */
    async rankByKeyword(query, asOf, limit = 20, predicate = null, minReinforced = null) {
        const lim = clampLimit(limit);
        const params = [query]; // $1 = query text
        ...
        if (minReinforced && minReinforced > 0)
            clauses.push(`(SELECT count(*) FROM fact_sources fs WHERE fs.fact_id = f.id) >= $${params.push(minReinforced)}`);
        const { rows } = await this.pool.query(`SELECT f.id,
              ts_rank(${FACT_TSVECTOR}, plainto_tsquery('english', $1)) AS rank
       FROM facts f
       JOIN entities subj ON subj.id = f.subject_id
       JOIN entities obj  ON obj.id  = f.object_id
       WHERE ${clauses.join(" AND ")}
       ORDER BY rank DESC LIMIT ${lim}`, params);
        return rows.map((r) => r.id);
    }
    /**
     ...
     */
    async history(subjectId, predicate) {
       ...
       ORDER BY COALESCE(f.valid_at, f.created_at) ASC, f.created_at ASC`, params);
     ...
     */
    async baselineCandidates(queryEmbedding, k = 5) {
        const { rows } = await this.pool.query(`SELECT obj.name AS object, f.valid_at, f.created_at
       FROM facts f JOIN entities obj ON obj.id = f.object_id
       WHERE f.embedding IS NOT NULL
       ORDER BY f.embedding <=> $1::vector
       LIMIT ${clampLimit(k)}`, [formatVector(queryEmbedding)]);
        return rows.map((r) => ({
            object: r.object,
            validAt: r.valid_at ?? null,
            createdAt: r.created_at,
    ...
    async loadRecalledByIds(ids) {
        if (ids.length === 0)
            return new Map();
        const { rows } = await this.pool.query(`${RECALL_SELECT} WHERE f.id = ANY($1::uuid[])`, [ids]);
        return new Map(rows.map((row) => [row.id, mapRecalledRow(row)]));
    }
    /**
     ...
     */
    async citingSourcesFor(factIds) {
        const out = new Map();
        if (factIds.length === 0)
            return out;
        const { rows } = await this.pool.query(`SELECT fs.fact_id, s.id, s.label
       FROM fact_sources fs
       JOIN sources s ON s.id = fs.source_id
       WHERE fs.fact_id = ANY($1::uuid[])
       ORDER BY s.created_at ASC, s.id ASC`, [factIds]);
        for (const r of rows) {
            const factId = r.fact_id;
            const list = out.get(factId) ?? [];
            list.push({ id: r.id, label: r.label ?? null });
            out.set(factId, list);
        }
        return out;
    }
    ...
    async graphStats() {
        const [{ rows: ent }, { rows: src }, { rows: factRows }, { rows: predRows }] = await Promise.all([
         ...
         FROM facts`),
            this.pool.query(`SELECT predicate,
                count(*)::int AS total,
                count(*) FILTER (WHERE expired_at IS NULL)::int AS current
         FROM facts
         GROUP BY predicate
        ...
        const total = factRows[0]?.total ?? 0;
        const current = factRows[0]?.current ?? 0;
        return {
            entities: ent[0]?.n ?? 0,
            sources: src[0]?.n ?? 0,
    ...
    async listEntities(opts = {}) {
        ...
        const { rows } = await this.pool.query(`SELECT e.id, e.name,
    ...
    async listSources(opts = {}) {
        const lim = clampLimit(opts.limit ?? 50);
        const { rows } = await this.pool.query(`SELECT s.id, s.label, s.text, s.created_at,
