...
import { z } from "zod";
import { remember } from "../pipeline.js";
...
export function createMcpServer(deps) {
    ...
    server.registerTool("remember", {
        title: "Remember",
        description: "Ingest a chunk of text as a Source: extract Facts, resolve Entities, and " +
    ...
    server.registerTool("preview", {
        title: "Preview",
        description: "Dry-run of `remember`: report what ingesting the text WOULD do — Facts it " +
    ...
    server.registerTool("recall", {
        title: "Recall",
        description: "Return ranked Facts matching the query — Current by default, or whatever " +
        ...
        if (as_of && Number.isNaN(asOf.getTime()))
            return errorResult(`invalid as_of date: ${as_of}`);
        try {
            const facts = await recall({ store: deps.store, provider: deps.provider }, query, {
                asOf,
                predicate,
                limit,
    ...
    server.registerTool("history", {
        title: "History",
        description: "Return the full Supersession chain for a subject (past and present Facts), " +
    ...
    server.registerTool("changes", {
        title: "Changes",
        description: "Return Facts whose transaction time changed since a date — those the memory " +
        ...
        if (Number.isNaN(sinceDate.getTime()))
            return errorResult(`invalid since date: ${since}`);
        try {
    ...
    server.registerTool("stats", {
        title: "Stats",
        description: "Return a read-only snapshot of the graph: Entity and Source counts, Fact " +
    ...
    server.registerTool("entities", {
        title: "Entities",
        description: "List the Entities in the graph, each with how many Current Facts touch it " +
    ...
    server.registerTool("sources", {
        title: "Sources",
        description: "List the ingested Sources, newest first — each with its label, ingest time, " +
