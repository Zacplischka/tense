...
| Engineering discipline | [230 tests across 46 spec files](../README.md#tested-against-a-real-database-not-mocks) — 28 of them integration tests against a real pgvector Postgres (not mocks), the rest pure-logic unit tests on the supersession rules; the whole `pnpm check` gate runs in CI on every push |
